module SourcedStore
  class PGBackend
    class Setup
      ...
      def self.run(db : DB::Database, logger : Logger)
        logger.info db.exec(SQL_SCHEMA)
        logger.info db.exec(SQL_EXTENSIONS)
        logger.info db.exec(SQL_CREATE_EVENTS_TABLE)
        logger.info db.exec(SQL_CREATE_INTERNAL_EVENTS_TABLE)
        logger.info db.exec(SQL_CREATE_CATEGORIES_TABLE_SQL)
        logger.info db.exec(SQL_CREATE_CATEGORIES_TO_EVENTS_TABLE_SQL)
        logger.info db.exec(SQL_FN_HASH64)
        logger.info db.exec(SQL_FN_READ_CATEGORY)
        logger.info db.exec(SQL_FN_READ_INTERNAL_STREAM)
        logger.info db.exec(SQL_PROC_COMPACT_STREAMS)
